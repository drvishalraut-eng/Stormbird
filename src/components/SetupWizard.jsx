...
//
// DATA DIR FLOW:
//   User picks a folder → wizard calls app:setDataDir → main writes
//   stormbird-config.json → app relaunches with new DATA_DIR.
//   If user leaves it blank, the default (next to exe) is used.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function SetupWizard({ onComplete }) {
  ...
  /**
   * Save the chosen dataDir. If blank, skip (use default).
   * If a path was entered, write it to config and relaunch.
   */
  const handleSaveDir = async () => {
    const trimmed = dataDir.trim();
    if (!trimmed) {
      // No path chosen — use the default, just move on
      next();
      return;
    }
    setSavingDir(true);
    setError('');
    const result = await window.sb?.appControl.setDataDir(trimmed);
    setSavingDir(false);
    if (!result || !result.success) {
      setError(result?.error || 'Could not save data folder. Check the path and try again.');
    }
    // If success, app relaunches — no need to call next()
  };
        ...
        {step === 1 && (
          <>
            <div style={S.header}>
              ...
              <p style={sub}>
                Choose a folder on your USB drive or local disk. All .eml files and the database will be stored here.
              </p>
            </div>
            <div style={S.body}>
              <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                <input
                  value={dataDir}
                  onChange={e => { setDataDir(e.target.value); setError(''); }}
                  placeholder="E:\Stormbird-Data  or  C:\Users\YourName\Stormbird-Data"
                  style={inputStyle()}
                />
              ...
              {dataDir.trim() && (
                <div style={{
                  ...
                  ⚡ Stormbird will restart automatically to apply the new data folder.
                </div>
              )}
              {error && (
                <p style={{ fontSize: 11, color: 'var(--cat-error)', margin: '8px 0 0' }}>⚠ {error}</p>
              )}
            </div>
            <div style={S.footer}>
              <Btn muted onClick={() => { setError(''); setStep(0); }}>← Back</Btn>
              <div style={{ flex: 1 }} />
              <Btn onClick={() => { setDataDir(''); next(); }}>Use default</Btn>
              <Btn accent onClick={handleSaveDir} disabled={savingDir}>
                {savingDir ? '⟳ Saving…' : dataDir.trim() ? 'Save & restart →' : 'Continue →'}
              </Btn>
            </div>
          </>
        )}
        ...
        {step === 2 && (
          <>
            ...
            <div style={S.body}>
              <div style={{
                background: 'var(--accent-dim)', border: '1px solid var(--border-accent)',
                padding: '8px 12px', marginBottom: 16, fontSize: 11,
              ...
              <input
                value={email}
                onChange={e => { setEmail(e.target.value); setError(''); setTestResult(null); }}
              ...
              <input
                type="password"
                value={password}
                onChange={e => { setPassword(e.target.value); setError(''); setTestResult(null); }}
                placeholder="xxxx xxxx xxxx xxxx"
                style={{ ...inputStyle(), marginBottom: 12 }}
              />

              {error && (
                <p style={{ fontSize: 11, color: 'var(--cat-error)', margin: '0 0 8px' }}>⚠ {error}</p>
              )}
              {testResult?.success && (
                <p style={{ fontSize: 11, color: 'var(--green)', margin: '0 0 8px' }}>✓ Connection successful</p>
              )}

              <Btn onClick={handleTestImap} disabled={testing || !email || !password}>
                {testing ? '⟳ Testing…' : 'Test connection'}
              </Btn>
            </div>
            <div style={S.footer}>
              <Btn muted onClick={() => { setError(''); setStep(1); }}>← Back</Btn>
              <div style={{ flex: 1 }} />
              <Btn onClick={next}>Skip for now</Btn>
              <Btn accent onClick={handleAddAccount} disabled={saving || !email || !password}>
          ...
          <>
            <div style={{ ...S.header, textAlign: 'center', paddingBottom: 0 }}>
              ...
              <p style={{ ...sub, textAlign: 'center' }}>
                Stormbird is ready. Click Sync in the sidebar to start downloading your emails.
              </p>
            </div>
            <div style={{ ...S.body, textAlign: 'center' }}>
              <p style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 2 }}>
                ...
                <strong style={{ color: 'var(--text-second)' }}>⏏</strong> — Safe Eject before removing USB
              </p>
            </div>
            <div style={S.footer}>
              <div style={{ flex: 1 }} />
